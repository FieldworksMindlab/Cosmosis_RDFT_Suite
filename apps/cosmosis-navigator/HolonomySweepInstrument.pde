...
class HoloSweepInstrumentState {
  ...
  int     orientStage        = 0;       // 0 idle, 1 await completion, 2 await rearm
  int     orientStageEnterMs = 0;
  boolean orientWasRunning   = false;   // true while the current orient loop is sweeping
  boolean orientLoopSealed   = false;   // private seal latch; CSV logger never touches this
  ...
  int     orientLastResult   = 0;       // 0 none/running, 1 completed OK, -1 aborted
  float   orientTagArea      = 0;       // area of the batch currently running
  final int ORIENT_REARM_TIMEOUT_MS = 4000;
  ...
  void orientLaunchOrAbort() {
    ...
    orientTagArea = abs((orientA1 - orientA0) * (orientD1 - orientD0));
    ...
    orientPairDone = 0;
    orientLoopSeq = 0;
    orientWasRunning = false;
    orientLoopSealed = false;
  ...
  void orientTick() {
    ...
    if (orientStage == 1) {
      updateOrientationProgressStatus();
      if (orientLoopSealed && orientWasRunning && !running) {
        orientLoopSealed = false;
        orientWasRunning = false;
    ...
    if (orientStage == 2) {
      boolean rearmed = (fsLifecycleState == FS_STATE_ARMED)
                      || (fsLifecycleState == FS_STATE_COOLDOWN)
                      || (fsLifecycleState == FS_STATE_RELEASING);
  ...
  void orientFireCurrentLoop() {
    ...
    // orientTagArea is set by the launcher; keep it stable for the whole batch.
    orientLoopSealed = false;
  ...
  boolean orientLaunchWith(float a0, float a1, int d0, int d1, float legT, float dwellT, int pairs, float areaTag) {
    if (orientRunActive) return false;
    if (!envFloquetFrozen) {
      statusMsg = "LADDER: environment not frozen - cannot launch batch.";
      return false;
    }

    orientA0 = constrain(a0, 1.0, 4.5);
    orientA1 = constrain(a1, 1.0, 4.5);
    orientD0 = round(constrain(d0, 0, 7));
    orientD1 = round(constrain(d1, 0, 7));
    orientLegT = constrain(legT, 2.0, 300.0);
    orientDwell = constrain(dwellT, 0.0, 60.0);
    orientPairTarget = round(constrain(pairs, 1, 50));
    orientTagArea = areaTag;

    if (hsFieldA0 != null) hsFieldA0.setText(safeNf(orientA0, 1, 2));
    if (hsFieldA1 != null) hsFieldA1.setText(safeNf(orientA1, 1, 2));
    if (hsFieldD0 != null) hsFieldD0.setText(str(orientD0));
    if (hsFieldD1 != null) hsFieldD1.setText(str(orientD1));
    if (hsFieldLegTime != null) hsFieldLegTime.setText(safeNf(orientLegT, 1, 1));
    if (hsFieldDwell != null) hsFieldDwell.setText(safeNf(orientDwell, 1, 1));
    if (hsFieldPairs != null) hsFieldPairs.setText(str(orientPairTarget));

    if (orientA0 == orientA1 || orientD0 == orientD1) {
      statusMsg = "LADDER: zero-area geometry for area " + safeNf(areaTag, 1, 1);
      return false;
    }
    ...
    orientStage = 1;
    orientStageEnterMs = millis();
    orientStatus = "orientation: run " + orientRunId + " area "
      + safeNf(areaTag, 1, 1) + " starting (" + orientPairTarget + " pairs)";
    statusMsg = orientStatus;
    updateOrientButton();
    orientFireCurrentLoop();
    return true;
  ...
  int orientTotalSeqCount() {
    return max(1, orientPairTarget * 2);
  }

  String orientationProgressSummary() {
    if (!orientRunActive) return orientStatus;
    String dir = orientPhaseReverse ? "REV" : "FWD";
    int seqHuman = constrain(orientLoopSeq + 1, 1, orientTotalSeqCount());
    int legHuman = constrain(legIndex + 1, 1, 4);
    int pct = round(constrain(legProgress, 0, 1) * 100.0);
    if (orientStage == 1 && running) {
      return "orientation run " + orientRunId + " " + dir
        + " seq " + seqHuman + "/" + orientTotalSeqCount()
        + " pair " + (orientPairDone + 1) + "/" + orientPairTarget
        + " leg " + legHuman + "/4 " + pct + "%";
    }
    if (orientStage == 2) {
      return orientStatus;
  ...
  void updateOrientationProgressStatus() {
    if (!orientRunActive) return;
    orientStatus = orientationProgressSummary();
    statusMsg = orientStatus;
  }
  ...
  void sealLoop() {
    ...
    if (berryTracker != null) {
      ...
      updateFourthSphere(berryTracker.holonomyRaw, lastLoopArea);
    }
    floquetCrossingSealed(lastHolonomyRad, lastLoopArea);
    loopClosedFlag = 1;
    if (orientRunActive && orientWasRunning) {
      orientLoopSealed = true;
    }
    logLoopResult();
