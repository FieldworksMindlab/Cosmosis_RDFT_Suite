...
PROCESSING_SKETCH_FILE="${PROCESSING_SKETCH_FILE:-Cosmosis_Navigator.pde}"
SUPERCOLLIDER_APP="${SUPERCOLLIDER_APP:-SuperCollider}"
SUPERCOLLIDER_FILE="${SUPERCOLLIDER_FILE:-ColdSun_ShimmerB.scd}"
...
if [[ "$LAUNCHPAD_BRIDGE" == "1" && -f "launchpad_bridge.swift" ]]; then
  ...
  if command -v "$SWIFTC_BIN" >/dev/null 2>&1; then
    ...
    if [[ ! -x "launchpad_bridge" || "launchpad_bridge.swift" -nt "launchpad_bridge" ]]; then
      CLANG_MODULE_CACHE_PATH="$XDG_CACHE_HOME/clang-module-cache" \
      "$SWIFTC_BIN" -module-cache-path "$XDG_CACHE_HOME/swift-module-cache" \
        -O launchpad_bridge.swift -o launchpad_bridge \
    ...
    if ! strings "$PROJECT_DIR/launchpad_bridge" | grep -Fq "$LAUNCHPAD_BRIDGE_MARKER"; then
      echo "Launchpad bridge is missing current static-LED marker; refusing to launch stale binary."
      exit 1
    fi
    for stale in "RDFT Launchpad page CC" "/lpx" "GALAXY SHEAR" "RDFT_LPX_PAINT" "static-8 scale-page"; do
      if strings "$PROJECT_DIR/launchpad_bridge" | grep -Fq "$stale"; then
        echo "Launchpad bridge still contains stale marker '$stale'; refusing to launch stale binary."
        exit 1
      fi
    done
    nohup env \
      CLANG_MODULE_CACHE_PATH="$XDG_CACHE_HOME/clang-module-cache" \
      "$PROJECT_DIR/launchpad_bridge" \
    ...
    echo "swiftc not found; Launchpad bridge disabled. macOS sees the Launchpad, but Processing Java cannot read it directly."
  fi
fi

if [[ "$HOLO_OPEN_SUPERCOLLIDER" == "1" ]]; then
  open -a "$SUPERCOLLIDER_APP" "$PROJECT_DIR/$SUPERCOLLIDER_FILE" 2>/dev/null || \
    echo "Could not open SuperCollider app '$SUPERCOLLIDER_APP'. Open $SUPERCOLLIDER_FILE manually."
fi

if [[ "$HOLO_OPEN_PROCESSING" == "1" ]]; then
  if [[ -f "$PROJECT_DIR/$PROCESSING_SKETCH_FILE" ]]; then
    open -a "$PROCESSING_APP" "$PROJECT_DIR/$PROCESSING_SKETCH_FILE" 2>/dev/null || \
      echo "Could not open Processing app '$PROCESSING_APP'. Open $PROCESSING_SKETCH_FILE manually."
  else
    echo "Missing Processing sketch file: $PROJECT_DIR/$PROCESSING_SKETCH_FILE"
  fi
fi
