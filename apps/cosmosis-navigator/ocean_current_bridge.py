...
def derive_dynamics(levels: list[float], temp_vals: list[float], current: float) -> tuple[float, float, float]:
...
def fetch_ocean(station: str) -> tuple[float, float, float, float, float, float, float, float, float, float]:
    ...
    if temp_rows:
        ...
        if vals:
            temp_c = vals[-1]
    velocity, accel, energy = derive_dynamics(levels, vals, current)
    live = 1.0
