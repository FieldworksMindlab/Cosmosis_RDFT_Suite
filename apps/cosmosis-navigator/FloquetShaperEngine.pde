...
final float FS_INTEGRATED_MS        = 40000;
final float FS_INTEGRATED_ENV       = 0.22;
final float FS_MAX_LOCK_MS          = 35000;
final float FS_RELEASE_MS           = 11000;
final float FS_COOLDOWN_MS          = 6000;
final float FS_SETTLE_IMPROVE_EPS   = 0.004;
final float FS_PLATEAU_MS           = 6500;
final float FS_EBB_FLOOR            = 0.12;
final float FS_RESONATE_LEVEL       = 0.45;
final float FS_PEAK_LEVEL           = 0.90;
final float FS_INJECT_HOLD_MS       = 2500;
final float FS_RESONATE_MS          = 14000;
final float FS_EBB_MS               = 11000;
final float FS_REASSERT_RAMP_MS     = 3000;
final float FS_RAMP_UP_RATE         = 0.06;
final float FS_EASE_RATE            = 0.025;
final float FS_READINESS_MIN        = 0.40;
final float FS_BAD_FIELD_MS         = 9000;
final float FS_KILL_MUTE_MS         = 9000;
...
// Cooperative-dance sub-phase inside the sustained INTEGRATED state.
final int FS_DANCE_RESONATE = 0;
final int FS_DANCE_EBB      = 1;
final int FS_DANCE_REASSERT = 2;
int fsDancePhase            = FS_DANCE_RESONATE;
float fsDancePhaseEnterMs   = 0;
float fsBadFieldSinceMs     = -1;
boolean fsManualAttempt     = false;
float fsMuteUntilMs         = 0;
...
void updateTargetWeights() {
  // Ocean data: high current and wave energy favour coupling and phase lock.
  float oceanActivity = 0;
  if (oceanBridge != null) {
    oceanActivity = constrain(
      oceanBridge.energyNorm * 0.68 + oceanBridge.currentNorm * 0.32,
      0, 1);
  }
...
// =============================================================================
float fieldReadiness() {
  float r = fsLockScoreRaw / max(0.001, FS_LOCK_THRESHOLD);
  return constrain(r, FS_READINESS_MIN, 1.0);
}

void floquetManualAttempt() {
  fsManualAttempt = true;
}

void floquetKill() {
  fsManualAttempt = false;
  fsMuteUntilMs = millis() + FS_KILL_MUTE_MS;
  enterFloquetState(FS_STATE_RELEASING, millis());
}
...
void updateFloquetLifecycle(float nowMs) {
  ...
  // Autonomous lock: engage when the field itself is coherent enough (sense-and-inject
  // at the field root), independent of a holonomy sweep. A running sweep still forces
  // LOCKING via floquetSweepStarted(), so the crossing path remains an extra entrance.
  boolean lockCandidate =
    (fsLockScoreRaw > FS_LOCK_THRESHOLD || (recentlyDepartedCoherence && fsLockScoreRaw > (FS_LOCK_THRESHOLD - 0.06)));

  switch(fsLifecycleState) {
    case FS_STATE_IDLE:
      fsCouplingEnvelope = approach(fsCouplingEnvelope, 0.0, FS_EASE_RATE);
      fsGateOpen = false;
      break;
    ...
    case FS_STATE_ARMED:
      ...
      fsCouplingEnvelope = approach(fsCouplingEnvelope, 0.0, FS_EASE_RATE);
      fsGateOpen = false;
      if ((lockCandidate && nowMs > fsMuteUntilMs) || fsManualAttempt) {
        fsManualAttempt = false;
        fsLockCandidateStartMs = -1;
        enterFloquetState(FS_STATE_LOCKING, nowMs);
      } else if (lockCandidate) {
    ...
    case FS_STATE_LOCKING:
      fsCouplingEnvelope = approach(fsCouplingEnvelope,
        max(FS_PEAK_LEVEL * fieldReadiness(), FS_EBB_FLOOR), FS_RAMP_UP_RATE);
      if (fsCouplingEnvelope >= (FS_PEAK_LEVEL * 0.82) || (nowMs - fsStateEnterMs) > FS_LOCK_RAMP_MS) {
        enterFloquetState(FS_STATE_INJECTING, nowMs);
      }
      break;

    case FS_STATE_INJECTING:
      fsCouplingEnvelope = approach(fsCouplingEnvelope, FS_PEAK_LEVEL * fieldReadiness(), FS_RAMP_UP_RATE);
      if ((nowMs - fsStateEnterMs) > FS_INJECT_HOLD_MS) {
        fsDancePhase = FS_DANCE_RESONATE;
        fsDancePhaseEnterMs = nowMs;
        fsBadFieldSinceMs = -1;
    ...
    case FS_STATE_INTEGRATED: {
      ...
      if (fsDancePhase == FS_DANCE_RESONATE) {
        baseTarget = FS_RESONATE_LEVEL;
        if ((nowMs - fsDancePhaseEnterMs) > FS_RESONATE_MS) {
          fsDancePhase = FS_DANCE_EBB;
          fsDancePhaseEnterMs = nowMs;
        }
      } else if (fsDancePhase == FS_DANCE_EBB) {
        baseTarget = FS_EBB_FLOOR;
        if ((nowMs - fsDancePhaseEnterMs) > FS_EBB_MS) {
          fsDancePhase = FS_DANCE_REASSERT;
          fsDancePhaseEnterMs = nowMs;
        }
      } else {
        baseTarget = FS_PEAK_LEVEL;
        if ((nowMs - fsDancePhaseEnterMs) > FS_REASSERT_RAMP_MS) {
          fsDancePhase = FS_DANCE_RESONATE;
          fsDancePhaseEnterMs = nowMs;
        }
      }
      float effTarget = FS_EBB_FLOOR + ((baseTarget - FS_EBB_FLOOR) * fieldReadiness());
      effTarget = max(effTarget, FS_EBB_FLOOR);
      float rate = (effTarget > fsCouplingEnvelope) ? FS_RAMP_UP_RATE : FS_EASE_RATE;
      fsCouplingEnvelope = approach(fsCouplingEnvelope, effTarget, rate);

      if ((nowMs - fsIntegratedEnterMs) > FS_INTEGRATED_MS) {
        enterFloquetState(FS_STATE_RELEASING, nowMs);
      } else if (fsLockScoreRaw < (FS_LOCK_THRESHOLD * 0.45)) {
        if (fsBadFieldSinceMs < 0) fsBadFieldSinceMs = nowMs;
        if ((nowMs - fsBadFieldSinceMs) > FS_BAD_FIELD_MS) {
          enterFloquetState(FS_STATE_RELEASING, nowMs);
        }
      } else {
        fsBadFieldSinceMs = -1;
    ...
    case FS_STATE_RELEASING:
      ...
      fsGateOpen = false;
      fsCouplingEnvelope = approach(fsCouplingEnvelope, 0.0, FS_EASE_RATE);
      if ((nowMs - fsStateEnterMs) > FS_RELEASE_MS) {
        enterFloquetState(FS_STATE_COOLDOWN, nowMs);
      }
      break;
