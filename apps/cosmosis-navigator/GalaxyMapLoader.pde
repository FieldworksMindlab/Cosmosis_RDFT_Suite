...
void drawGalaxyPanel(int x, int y, int w, int h) {
  ...
  drawMiniReadout(rx, ry + 48, rw, "boundary",    galaxyBoundaryPersist, safeNf(galaxyBoundaryPersist,1,2));
  drawMiniReadout(rx, ry + 72, rw, "blend",       galaxyBlend,           safeNf(galaxyBlend,1,2));
  int bx = x + 9;
  int by2 = y + h - 18;
  fill(18, 12, 32, 180);
  noStroke();
