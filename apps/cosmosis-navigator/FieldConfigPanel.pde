...
class FieldConfigPanel extends PApplet {
  ...
  void drawFCHeader() {
    ...
    String solarStr = syncSolarLive
      ? ("SOLAR LIVE  x=" + fmt(syncSolarXray, 2) + " w=" + fmt(syncSolarWind, 2) + " kp=" + fmt(syncSolarKp, 1))
      : "SOLAR: WAITING";
  ...
  void drawEGPanel() {
    ...
    text("EMERGENT FIELD FACTOR  [α=" + fmt(syncAlpha, 2) + "  R=" + fmt(syncDepth, 0) + "  m=" + fmt(syncOrbM, 2) + "]", sx, cy);
    cy += 13;
    ...
    text("g_eff = " + fmt(egF, 3), sx + sw, cy + 12);
    cy += 26;
    ...
    drawSlider(SL_VOLTAGE, egInjectedVoltage, 0, 100,
               ...
               egSolarAuto ? "SOLAR AUTO" : (fmt(egInjectedVoltage, 1) + " kV"));
    cy += 24;

    // Effective voltage (injection + emergent combined) readout
    fill(TEXT_COL); textSize(8); textAlign(LEFT, TOP);
    text("EFFECTIVE:  " + fmt(syncVoltageEG, 2) + " kV   [injection + emergent composite]", sx, cy);
    cy += 20;
  ...
  void drawUFOPanel() {
    ...
    text("α=" + fmt(impliedAlpha, 2), sx + sw, cy + 10);
    cy += 20;
    ...
    fill(TEXT_COL); textSize(8); textAlign(RIGHT, TOP);
    text("R=" + fmt(impliedDepth, 1), sx + sw, cy + 10);
    cy += 24;
  ...
  void drawFieldStateStrip() {
    ...
    for (int i = 0; i < labels.length; i++) {
      ...
      text(fmt(vals[i], 3), cx + i * colW, my + 5);
      textSize(8);
    }
    ...
    text("g_eff " + fmt(syncEmergentEG, 3)
         + "   V=" + fmt(syncVoltageEG, 1) + "kV"
         + "   UFO-T=" + fmt(constrain(ufoTransition, 0, 1), 2),
         x + w - 10, my);
    ...
    if (syncSurfaceReady) {
      ...
      text("SURFACE  curv=" + fmt(syncSurfaceCurv, 2)
           + "  Ls=" + fmt(syncLandauerS, 3)
           + "  coh=" + fmt(syncLandauerCoherence, 2)
