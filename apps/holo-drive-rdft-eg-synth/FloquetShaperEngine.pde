...
void floquetShaperUpdate() {
  ...
  fsAlphaNow = constrain(baseAlpha + FS_STRENGTH * fsCouplingEnvelope * fsDriveNow, 1.1, 4.5);
...
void updateTargetWeights() {
  // Ocean data: current, velocity, and energy favour coupling and phase lock.
  float oceanActivity = 0;
  if (oceanBridge != null) {
    oceanActivity = constrain(
      oceanBridge.energyNorm * 0.58 + oceanBridge.currentNorm * 0.24 + oceanBridge.velocityNorm * 0.18,
      0, 1);
  }
