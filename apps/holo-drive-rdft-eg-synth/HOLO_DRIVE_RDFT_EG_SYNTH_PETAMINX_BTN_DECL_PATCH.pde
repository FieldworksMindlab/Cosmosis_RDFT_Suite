...
// Window/Layout
final int W = 1580;
final int H = 912;  // taller by ~72 px so the Solar Node can breathe without overlapping controls
...
void applyMidiPilotUpdate() {
  ...
  if (noteEvents != null) {
    for (int i = 0; i < noteEvents.length; i++) {
      handleMidiNoteEvent(round(noteEvents[i][0]), noteEvents[i][1], noteEvents[i][2] > 0.5, round(noteEvents[i][3]));
    }
  }
  ...
  if (!midiPolyMode) {
    ...
    midiPilotStatus = "MIDI scalar " + note + " -> " + safeNf(freq, 1, 2) + " Hz" + (held > 1 ? (" chord x" + held) : "");
  } else {
    midiPilotHelper.consumePendingValues();
    updateMidiPolyMemory();
    midiPilotStatus = "MIDI poly voices=" + midiHeldCount + " mem=" + safeNf(midiPolyMemoryCharge, 1, 2);
  }
}
...
void handleMidiNoteEvent(int note, float velocity, boolean on, int heldFromDevice) {
  ...
  if (midiPolyMode) sendMidiPolyNoteToSC(note, velocity, on);
}
...
void updateMidiPolyMemory() {
  ...
  if (held > 0 && low >= 0) {
    ...
    midiPolyMemoryCharge = constrain(midiPolyMemoryCharge + 0.010 * held * (1.0 - midiPolyReleasePressure), 0, 1);
  } else {
    ...
    midiPolyMemoryCharge = max(0, midiPolyMemoryCharge - (0.010 + midiPolyReleasePressure * 0.035));
  }

  float rootEase = 0.002 + midiPolyMemoryCharge * 0.012 * (1.0 - midiPolyReleasePressure);
  if (held > 0 && midiPolyMemoryCharge > 0.22 && sliderRootFreq != null) {
    float nextRoot = lerp(sliderRootFreq.getValue(), midiPolyRootTargetFreq, rootEase);
    sliderRootFreq.setValue(nextRoot);
    if (fieldRootFreq != null && !fieldRootFreq.isFocus()) fieldRootFreq.setText(safeNf(nextRoot, 1, 1));
    midiPolyLastRootApplied = nextRoot;
  }
...
void sendMidiPolyNoteToSC(int note, float velocity, boolean on) {
  ...
  msg.add(getSliderValueSafe("midiPolyVolume", 0.56));
...
void setup() {
  surface.setTitle("Recursive Differentiation Framework");
  colorMode(RGB, 255);
...
void updateOrbitEventDetector() {
  ...
  orbitEccentricity = constrain((maxR - minR) / max(0.001, maxR + minR), 0, 1);
  ...
  orbitResonanceScore = constrain((1.0 - abs(orbitLobeEstimate - 3) / 3.0) * orbitEccentricity * constrain(peaks / 8.0, 0, 1), 0, 1);
...
void computeFoundationalFieldMetrics(float[][] field, float finest, float dominantVal, float avgBright, float maxBright, float orbSpeed, float orbRadius) {
  ...
  if (midiPolyMode || midiPolyMemoryCharge > 0.001) {
...
class OceanDataBridge {
  ...
  void updateFieldInterpreter() {
    ...
    oceanColor = constrain(map(rawTempC, 0, 32, 0, 1) * 0.55 + energyNorm * 0.45, 0, 1);
...
void SPAWNDRONE(int val) {
  ...
  float v = getSliderValueSafe("droneVolume", 0.18);
...
mMsg.add(getSliderValueSafe("droneVolume", 0.18));
  mMsg.add(getSliderValueSafe("arpVolume", 0.25));
  ...
  mMsg.add(choirOnState ? getSliderValueSafe("choirVolume", 0.12) : 0.0f);
...
mMsg.add(getSliderValueSafe("ecoChirpLevel", 0.38));
mMsg.add(getSliderValueSafe("ecoCallLevel", 0.42));
mMsg.add(getSliderValueSafe("ecoGrowthLevel", 0.30));
mMsg.add(getSliderValueSafe("ecoFissureLevel", 0.28));
...
void setupExperimentLogging() {
  ...
  oscLogWriter.println(
    ...
    "adiabatic_param,berry_phase,berry_phase_principal,berry_winding_turns,berry_curvature_density,orient_run_id,orient_loop_seq,correlation_tau," +
...
void logOscStateFrame() {
  ...
  String line =
    ...
    // 20-23  volumes
    safeNf(getSliderValueSafe("droneVolume", 0.18), 1, 4) + "," +
    safeNf(getSliderValueSafe("arpVolume", 0.25), 1, 4) + "," +
    safeNf(getSliderValueSafe("glassVolume", 0.2), 1, 4) + "," +
    safeNf(getSliderValueSafe("choirVolume", 0.12), 1, 4) + "," +
    // 26-32  metrics
    ...
    safeNf(getSliderValueSafe("ecoChirpLevel", 0.38), 1, 4) + "," +
    safeNf(getSliderValueSafe("ecoCallLevel", 0.42), 1, 4) + "," +
    safeNf(getSliderValueSafe("ecoGrowthLevel", 0.30), 1, 4) + "," +
    safeNf(getSliderValueSafe("ecoFissureLevel", 0.28), 1, 4) + "," +
