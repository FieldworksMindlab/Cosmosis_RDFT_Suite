...
Default packet:
OCEAN:tide=<m>,temp_c=<C>,current=<0-1>,phase=<0-1>,thermal=<0-1>,range=<m>,velocity=<0-1>,accel=<0-1>,energy=<0-1>,memory=<0-1>,live=<0-1>
...
def derive_dynamics(levels: list[float], temp_vals: list[float], thermal: float, current: float) -> tuple[float, float, float, float]:
    ...
    if len(temp_vals) >= 2:
        thermal_gradient_raw = abs(temp_vals[-1] - temp_vals[max(0, len(temp_vals) - 6)])
    else:
        thermal_gradient_raw = 0.0
    thermal_gradient = max(0.0, min(1.0, thermal_gradient_raw / 2.0))
    ...
    return velocity, accel, thermal_gradient, energy


def fetch_ocean(station: str) -> tuple[float, float, float, float, float, float, float, float, float, float, float]:
    ...
    if temp_rows:
        ...
        if vals:
            temp_c = vals[-1]
    thermal = max(0.0, min(1.0, (temp_c - 2.0) / 28.0))
    velocity, accel, _thermal_gradient, energy = derive_dynamics(levels, vals, thermal, current)
    live = 1.0
    return tide, temp_c, current, phase, thermal, tide_range, velocity, accel, energy, 0.0, live


def simulated_ocean(t: float) -> tuple[float, float, float, float, float, float, float, float, float, float, float]:
    ...
    energy = max(0.0, min(1.0, current * 0.35 + velocity * 0.25 + accel * 0.15 + thermal * 0.15))
    return tide, temp_c, current, phase_wave, thermal, tide_range, velocity, accel, energy, 0.0, 1.0


def with_memory(values: tuple[float, float, float, float, float, float, float, float, float, float, float]) -> tuple[float, float, float, float, float, float, float, float, float, float, float]:
    global ocean_memory
    tide, temp_c, current, phase, thermal, tide_range, velocity, accel, energy, _memory, live = values
    ocean_memory = (0.96 * ocean_memory) + (0.04 * energy)
    return tide, temp_c, current, phase, thermal, tide_range, velocity, accel, energy, ocean_memory, live


def format_packet(values: tuple[float, float, float, float, float, float, float, float, float, float, float]) -> str:
    tide, temp_c, current, phase, thermal, tide_range, velocity, accel, energy, memory, live = values
    return (
        f"OCEAN:tide={tide:.4f},temp_c={temp_c:.3f},current={current:.4f},"
        f"phase={phase:.4f},thermal={thermal:.4f},range={tide_range:.4f},"
