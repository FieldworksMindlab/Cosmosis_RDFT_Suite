...
--------------------------------------------
  ...
  thermal_excitation    <- temperature / emission proxy
...
def make_map(name, display_name, description,
             ...
             thermal_excitation, boundary_persistence,
    ...
    return {
        ...
        "thermal_excitation":   clamp(thermal_excitation),
...
#                G-band luminosity as thermal proxy
# ===========================================================================
...
def fetch_milky_way():
    ...
    try:
        ...
        print(f"    density={mean_density:.3f}  floquet={floquet:.3f}  "
              f"thermal={thermal:.3f}  boundary={boundary:.3f}")
        return make_map(
            "milky_way", "Milky Way",
...
def fetch_andromeda():
    ...
    try:
        ...
        # Find IR (stellar mass proxy) and UV/X-ray (star formation / thermal)
        ...
        thermal    = norm_log(uv_flux, 1e-4, 50.0) if uv_flux > 0 else 0.38
...
def fetch_hubble_deep_field():
    ...
    # Thermal: very hot — X-ray background, AGN, high-z star formation
    thermal = 0.74
...
GALAXY_POOL = [
    # (name, display, morph_type, field_curv, floquet, thermal, boundary,
    ...
    ("NGC 1569",  "NGC 1569",  "IBm",  0.38, 0.55, 0.82, 0.36,
     5e8,   "Starburst dwarf — low mass but thermally intense, turbulent"),
    ...
    ("Arp 220",   "Arp 220",   "Irr",  0.85, 0.88, 0.95, 0.55,
     3e11,  "ULIRG merger — extreme thermal excitation, chaotic topology"),
...
def fetch_random_galaxy():
    ...
    try:
        ...
        # Adjust thermal from redshift (higher-z = less constrained thermally)
        th = clamp(th * (1 - z * 0.1))
        print(f"    NED redshift z={z:.4f}, adjusted thermal={th:.3f}")
    except Exception as e:
...
def write_manifest(maps):
    ...
    for m in maps:
        manifest["maps"][m["name"]] = {
            ...
            "thermal_excitation": m["thermal_excitation"],
